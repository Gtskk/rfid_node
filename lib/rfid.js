...
RfidNodejs.prototype.open = function(param) {
	...
	var two = {
		...
		offDataCallback: this.offDataCallback.bind(this),
		onDataGetCallback: this.onDataGetCallback.bind(this)
	};
	for(var i = 0; i < 2; i++) {
		for(var j in two){
			temp[j] = two[j];
...
// 在架数据
RfidNodejs.prototype.onDataGetCallback = function(input, callback){
